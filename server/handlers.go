...
func launchInteractiveCommand(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	var req LaunchInteractiveCommandRequest
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		http.Error(w, "Invalid request payload", http.StatusBadRequest)
		return
	}
	...
	result, err := LaunchInteractiveCommand(teamName, req)
	if err != nil {
		if err == sql.ErrNoRows {
			http.Error(w, "Team not found", http.StatusNotFound)
			return
		}
		log.Printf("Error launching interactive command: %v", err)
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
